...
if "%PGPASSWORD%"=="" set PGPASSWORD=your_password
if "%PGUSER%"=="" set PGUSER=postgres
if "%PGHOST%"=="" set PGHOST=localhost
if "%PGPORT%"=="" set PGPORT=5432
if "%PGDATABASE%"=="" set PGDATABASE=interfaith_db

echo Step 1: Creating database (if not exists)...
psql -U %PGUSER% -h %PGHOST% -p %PGPORT% -d postgres -c "CREATE DATABASE interfaith_db;" 2>nul
if %errorlevel% equ 0 (
