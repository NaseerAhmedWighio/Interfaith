...
export async function PUT(
  ...
  try {
    ...
    const hasPermission = await checkPermission(user.id, 'mission_content', 'update')
    if (!hasPermission) {
      return NextResponse.json(
        { error: 'You do not have permission to update mission content' },
        { status: 403 }
      )
    }
    ...
    if (userWithRole?.role === 'admin') {
      const missionContent = await prisma.missionContent.update({
        where: { id },
        data: { sectionKey, title, content, status: 'published' },
      })
      return NextResponse.json(missionContent)
    }

    const pendingEdit = await prisma.pendingEdit.create({
      data: {
        contentType: 'mission_content',
        contentId: id,
    ...
    return NextResponse.json(
      { error: 'Failed to update mission content' },
      { status: 500 }
    )
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const hasPermission = await checkPermission(user.id, 'mission_content', 'delete')
    if (!hasPermission) {
      return NextResponse.json(
        { error: 'You do not have permission to delete mission content' },
        { status: 403 }
      )
    }
    ...
    console.error('Error deleting mission content:', error)
    ...
    return NextResponse.json(
      { error: 'Failed to delete mission content' },
      { status: 500 }
    )
  }
}
