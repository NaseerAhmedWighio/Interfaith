...
const iconComponents: Record<string, any> = {
  BookOpen, MessageCircle, HandHeart, Heart, Users, Target, Globe2, Lightbulb,
}
  ...
  return (
    <div>
      ...
      <section className="py-12 sm:py-16 md:py-20 px-4 sm:px-6 bg-[#0B0F2A]">
        <div className="container mx-auto max-w-6xl">
          <div className="text-center mb-10 sm:mb-12 md:mb-14">
            <h2 className="text-2xl sm:text-3xl md:text-4xl heading-premium text-[#f5f3ee] mb-3 sm:mb-4 px-4">{fourPillars?.title || 'Four Pillars of Our Work'}</h2>
          ...
          <div className="grid md:grid-cols-2 gap-6 sm:gap-7 md:gap-8 mb-12 sm:mb-16 md:mb-20">
            {getCards('pillar').length > 0 ? getCards('pillar').map((card, i) => {
              ...
              <>
                ...
                <PillarCard icon={<HandHeart className="w-8 h-8" />} title="Service & Action" description="Faith without action is incomplete. We unite diverse communities through collaborative service projects that address real needs." features={['Interfaith volunteer initiatives', 'Community development projects', 'Disaster relief partnerships', 'Social justice advocacy']} color="#E07070" />
                <PillarCard icon={<Heart className="w-8 h-8" />} title="Spiritual Practice" description="Shared contemplative practices create heart connections that transcend intellectual understanding and theological differences." features={['Interfaith meditation gatherings', 'Peace prayer ceremonies', 'Sufi-inspired spiritual retreats', 'Contemplative practice workshops']} color="#D4A07B" />
              </>
            )}
          </div>
          ...
          <div className="card-premium p-12 text-center">
            ...
            <h3 className="text-3xl heading-premium text-[#f5f3ee] mb-6">{philosophy?.title || 'Our Philosophy'}</h3>
            {philosophy ? (
              philosophy.content.split('\n\n').filter(Boolean).map((p, i) => (
                <p key={i} className="text-xl text-premium leading-relaxed max-w-3xl mx-auto">{p}</p>
              ))
            ) : (
      ...
      <section className="py-20 px-6 sacred-pattern">
        <div className="container mx-auto max-w-6xl">
          <div className="text-center mb-16">
            <h2 className="text-4xl heading-premium text-[#f5f3ee] mb-4">{methodology?.title || 'Our Methodology in Action'}</h2>
      ...
      <section className="py-20 px-6 bg-[#0B0F2A]">
        <div className="container mx-auto max-w-6xl">
          <div className="text-center mb-16">
            <h2 className="text-4xl heading-premium text-[#f5f3ee] mb-4">{corePrinciples?.title || 'Core Principles'}</h2>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {getCards('principle').length > 0 ? getCards('principle').map((card, i) => {
              ...
              <>
                ...
                <PrincipleCard title="Heart-Centered Approach" description="We engage from the heart, not just the head. Spiritual connection and lived experience complement intellectual understanding." icon={<Heart className="w-10 h-10 text-rose-600" />} />
                <PrincipleCard title="Collaborative Spirit" description="We work with, not for, communities. Local leaders and participants co-create programs that reflect their unique contexts and needs." icon={<HandHeart className="w-10 h-10 text-amber-600" />} />
                <PrincipleCard title="Action-Oriented" description="Dialogue without action is incomplete. We translate understanding into concrete initiatives that create positive change." icon={<Target className="w-10 h-10 text-violet-600" />} />
                <PrincipleCard title="Global-Local Balance" description="We maintain a global vision while honoring local contexts, cultures, and specific interfaith dynamics in each community." icon={<Globe2 className="w-10 h-10 text-emerald-600" />} />
              </>
            )}
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-16 items-center">
            ...
            <div className="space-y-6">
              <div className="card-premium p-8">
                <h3 className="text-2xl heading-premium text-[#f5f3ee] mb-4">{successMetrics?.title || 'Success Metrics'}</h3>
                {successMetrics ? (
