...
export default function NewSacredText() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json()
        throw new Error(data.error || 'Failed to create sacred text')
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-[#f5f3ee]">Add Sacred Text</h1>
          <p className="text-premium-light mt-1 text-sm">Create a new sacred text entry</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="glass-effect rounded-xl p-4 sm:p-6 border border-[#c8a75e]/20 space-y-6">
        ...
        <div className="flex gap-3 pt-4">
          ...
          >
            ...
            {saving ? 'Creating...' : 'Create Sacred Text'}
          </button>
          <Link
            href="/admin/sacred-texts"
