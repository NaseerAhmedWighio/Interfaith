...
export default function EditMisconception({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function loadData() {
    try {
      const [misconceptionRes, traditionsRes] = await Promise.all([
        fetch(`/api/misconceptions/${resolvedParams.id}`),
        fetch('/api/traditions')
      ])

      if (!misconceptionRes.ok) throw new Error('Failed to load misconception')

      const misconceptionData = await misconceptionRes.json()
      const traditionsData = await traditionsRes.json()

      setFormData({
        traditionId: misconceptionData.traditionId || '',
        misconception: misconceptionData.misconception || '',
        truth: misconceptionData.truth || '',
        category: misconceptionData.category || 'general'
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/misconceptions/${resolvedParams.id}`, {
        method: 'PUT',
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || 'Failed to update misconception')
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-[#f5f3ee]">Edit Misconception</h1>
          <p className="text-premium-light mt-1 text-sm">Update misconception details</p>
        </div>
      </div>
